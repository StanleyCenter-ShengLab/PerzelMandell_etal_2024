...
##-
RunEnrichment<-function(scores=c(),df=NULL,genes="Genes",Score="ES",Terms=NULL,CleanTerms=NULL,maxSize=500,minSize=15)
{
if(is.null(Terms))
{
...
out<-data.frame(fgsea(Terms,scores,minSize=minSize,maxSize=maxSize,nproc=1))
...
RunEnrichment_GO_new<-function(scores = c(), df = NULL, genes = "Genes", Score = "ES",organism="mmu",orgDB=NULL, maxSize=500, minSize = 15)
{
print("Download GO")
...
out<-RunEnrichment(scores=scores,df=df,genes=genes,Score=Score,CleanTerms=tab,maxSize=maxSize,minSize=minSize)
print("Get ONTs")
...
dat = read.csv("path/to/DE/output.csv")
dat = dat[!is.na(dat$padj),]
go = RunEnrichment_GO_new(df = dat, genes = "X", Score = "stat")
